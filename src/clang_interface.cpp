...
addPrimitiveTypes(ApiFuzzerNew* afn)
{
    ...
    afn->addType(new PrimitiveType("uint32_t"));
    afn->addType(new PrimitiveType("int64_t"));
...
generateObjectInstructions(std::string type_name, std::string indent)
{
    ...
    instrs_str_vec.insert(instrs_str_vec.begin(), "fuzz::mfr_fuzz_start(\"" + new_obj->toStr() + "\");");
    instrs_str_vec.push_back("fuzz::mfr_fuzz_end(\"" + new_obj->toStr() + "\");");
